...
app = FastAPI(title="Agent知识库后端", version="3.0.0")
...
async def get_knowledge(submission_id: str):
    conn = sqlite3.connect(DB_PATH); conn.row_factory = sqlite3.Row; c = conn.cursor()
...
async def admin_page():
    return HTMLResponse((BASE_DIR / "admin.html").read_text(encoding="utf-8")) if (BASE_DIR / "admin.html").exists() else HTMLResponse("<h1>admin.html not found</h1>", 404)
